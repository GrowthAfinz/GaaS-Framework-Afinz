...
export const syncFrameworkActivities = async (
    ...
    const sqlBatch = frameworkActivities.map((activity) => {
        ...
        return {
            ...
            "Etapa de aquisição": activity.etapaAquisicao ?? activity.raw?.['Etapa de aquisição'],
            "Perfil de Crédito": activity.perfilCredito ?? activity.raw?.['Perfil de Crédito'],
            "Produto": activity.produto ?? activity.raw?.['Produto'],
            "Oferta": activity.oferta,
            "Promocional": activity.promocional ?? activity.raw?.['Promocional'],
            "SIGLA_Oferta": activity.raw?.['SIGLA.2'],
            "Oferta 2": activity.oferta2 ?? activity.raw?.['Oferta 2'],
            "Promocional 2": activity.promocional2 ?? activity.raw?.['Promocional 2'],
