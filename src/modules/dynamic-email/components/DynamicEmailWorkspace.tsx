...
const WeekReviewer = ({ selection, groups, issuesByRow, selectedId, onSelect, onEdit, onNewEmail, onDuplicate }: { selection: WeekSelection; groups: EditorialGroup[]; issuesByRow: Map<string, ValidationIssue[]>; selectedId: string; onSelect: (id: string) => void; onEdit: (id: string) => void; onNewEmail: () => void; onDuplicate: () => void }) => {
  const summaries = groups.map((group) => {
    ...
    const issues = rows.flatMap((row) => issuesByRow.get(row.__id) ?? []);
