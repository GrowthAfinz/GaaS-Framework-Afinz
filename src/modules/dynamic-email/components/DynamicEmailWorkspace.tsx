...
import { countConfiguredStrategyFields, strategyReadiness, type EmailStrategy, type ExternalReviewRun, type ExternalSuggestion, type ProductContext, type ProductGuardrail } from '../domain/management';
...
export const DynamicEmailWorkspace: React.FC = () => {
  ...
  const selectEmail = (id: string) => { setSelectedWeek(null); setSelectedId(id); };
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-8 gap-y-4">
        ...
        <div className="ml-auto flex flex-col items-end gap-2">
          <div className="flex items-center rounded-xl bg-white/10 p-1" role="tablist" aria-label="Área da Fábrica de E-mails">
            ...
            <button role="tab" aria-selected={mode === 'strategy'} onClick={() => setMode('strategy')} className={`rounded-lg px-3 py-2 text-xs font-bold transition ${mode === 'strategy' ? 'bg-white text-slate-900 shadow-sm' : 'text-cyan-50 hover:bg-white/10'}`}>Estratégia e guardrails</button>
      ...
      <Group id="email-briefing-workspace" orientation="horizontal" defaultLayout={workspaceDefaultLayout ?? { briefings: 20, content: 80 }} onLayoutChanged={onWorkspaceLayoutChanged} className="min-h-[720px] min-w-0 overflow-hidden" resizeTargetMinimumSize={{ coarse: 20, fine: 10 }}>
        ...
        <aside className="h-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="Caixa de briefings">
          ...
          <div className="max-h-[790px] overflow-y-auto p-2.5">
            ...
            {filteredGroups.length ? <BriefingTree groups={filteredGroups} selectedId={selected?.__id ?? ''} selectedWeek={selectedWeek} showArchived={showArchived} onSelect={selectEmail} onSelectWeek={(week) => { setSelectedWeek(week); const first = editorialGroups.find((group) => group.visibleRows.length && group.representative.__meta.partner === week.partner && group.representative.__meta.segment === week.segment && group.representative.__meta.weekKey === week.weekKey)?.representative; if (first) setSelectedId(first.__id); }} onManage={(id) => { const target = rows.find((row) => row.__meta.campaignGroupId === id && row.__meta.status !== 'archived') ?? rows.find((row) => row.__meta.campaignGroupId === id); if (target) setSelectedId(target.__id); setSignatureManagerOpen(true); }} onNewWeek={openNewWeek} onNewEmail={openNewBriefing} onDuplicateWeek={duplicateWeek} onArchiveWeek={(partner, segment, weekKey) => setWeekArchiveTarget({ partner, segment, weekKey })} onDuplicateEmail={duplicateGroup} onArchiveEmail={(groupId) => { const target = rows.find((row) => row.__meta.campaignGroupId === groupId && row.__meta.status !== 'archived'); if (target) { setSelectedId(target.__id); setDeleteOpen(true); } }}/> : <div className="px-4 py-10 text-center text-sm text-slate-500"><Search className="mx-auto mb-2 text-slate-300" size={24}/><p className="font-semibold text-slate-700">Nenhum briefing encontrado</p><p className="mt-1 text-xs">Ajuste a busca ou o filtro de status.</p></div>}
          </div>
        </aside>
        </Panel>
...
const WeekReviewer = ({ selection, groups, strategies, issuesByRow, selectedId, onSelect, onEdit, onNewEmail, onDuplicate }: { selection: WeekSelection; groups: EditorialGroup[]; strategies: EmailStrategy[]; issuesByRow: Map<string, ValidationIssue[]>; selectedId: string; onSelect: (id: string) => void; onEdit: (id: string) => void; onNewEmail: () => void; onDuplicate: () => void }) => {
...
const StrategyWorkspace = ({ strategies, contexts, guardrails, syncState, onRefresh, onSaved }: { strategies: EmailStrategy[]; contexts: ProductContext[]; guardrails: ProductGuardrail[]; syncState: string; onRefresh: () => void; onSaved: (strategy: EmailStrategy) => void }) => {
  ...
  const [draft, setDraft] = useState<EmailStrategy | null>(selected ?? null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState('');
  useEffect(() => { setDraft(selected ?? null); }, [selected?.id, selected?.version]);
  const update = (field: keyof EmailStrategy, value: string) => setDraft((current) => current ? { ...current, [field]: value } : current);
  const save = async () => { if (!draft) return; setSaving(true); setMessage(''); try { const saved = await saveEmailStrategy(draft); onSaved(saved); setDraft(saved); setMessage('Estratégia salva com nova versão auditável.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Falha ao salvar.'); } finally { setSaving(false); } };
  return <main className="mt-4 grid min-h-[720px] gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
    <aside className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><header className="border-b border-slate-200 p-4"><div className="flex items-center justify-between gap-2"><div><h2 className="font-bold text-slate-900">Estratégia da régua</h2><p className="text-xs text-slate-500">{strategies.length} e-mails · {syncState}</p></div><button type="button" onClick={onRefresh} className="rounded-lg border border-slate-200 p-2 text-slate-600" aria-label="Atualizar estratégia"><RefreshCw size={15}/></button></div></header><div className="max-h-[760px] space-y-2 overflow-auto p-3">{strategies.map((item) => { const readiness = strategyReadiness(item); return <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-xl border p-3 text-left ${selected?.id === item.id ? 'border-cyan-400 bg-cyan-50' : 'border-slate-200 bg-white hover:border-cyan-200'}`}><span className="block text-xs font-bold text-slate-900">{item.partner} · {segmentDisplayLabel(item.segment)}</span><span className="mt-1 block text-sm font-extrabold">{item.weekKey} · {item.sequence}</span><span className="mt-2 flex items-center justify-between text-[10px] text-slate-500"><span>{countConfiguredStrategyFields(item)}/7 campos estratégicos</span><span className={readiness.tone === 'danger' ? 'text-red-700' : readiness.tone === 'success' ? 'text-emerald-700' : 'text-amber-700'}>{readiness.label}</span></span></button>; })}</div></aside>
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><header className="border-b border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-slate-900">Contexto semântico e guardrails</h2><p className="text-xs text-slate-500">Camada gerencial separada das 36 colunas do SFMC.</p></div>{draft && <button type="button" onClick={() => void save()} disabled={saving} className="rounded-lg bg-[#07595b] px-4 py-2 text-xs font-bold text-white disabled:opacity-50"><Save className="mr-1.5 inline" size={14}/>{saving ? 'Salvando…' : 'Salvar nova versão'}</button>}</div></header><div className="max-h-[760px] overflow-auto p-4">{message && <div role="status" className="mb-3 rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs text-cyan-900">{message}</div>}{draft ? <><div className="grid gap-3 md:grid-cols-2"><SemanticField label="Papel na régua" value={draft.roleInRuler ?? ''} onChange={(value) => update('roleInRuler', value)}/><SemanticField label="Objetivo do e-mail" value={draft.emailObjective ?? ''} onChange={(value) => update('emailObjective', value)}/><SemanticField label="Mensagem-chave" value={draft.keyMessage ?? ''} onChange={(value) => update('keyMessage', value)}/><SemanticField label="Ação esperada" value={draft.expectedAction ?? ''} onChange={(value) => update('expectedAction', value)}/><SemanticField label="Proposta de valor" value={draft.valueProposition ?? ''} onChange={(value) => update('valueProposition', value)}/><SemanticField label="Benefício principal" value={draft.primaryBenefit ?? ''} onChange={(value) => update('primaryBenefit', value)}/><SemanticField label="Objeção trabalhada" value={draft.objectionAddressed ?? ''} onChange={(value) => update('objectionAddressed', value)}/><SemanticField label="Estratégia de CTA" value={draft.ctaStrategy ?? ''} onChange={(value) => update('ctaStrategy', value)}/></div><div className="mt-5 grid gap-4 xl:grid-cols-2"><ContextCard contexts={contexts} partner={draft.partner}/><GuardrailList contexts={contexts} guardrails={guardrails} partner={draft.partner}/></div></> : <EmptyManagementState/>}</div></section>
  </main>;
};

const SemanticField = ({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) => <label className="text-xs font-bold text-slate-700">{label}<textarea value={value} onChange={(event) => onChange(event.target.value)} rows={3} className="mt-1 w-full resize-y rounded-lg border border-slate-200 px-3 py-2 text-sm font-normal outline-none focus:border-cyan-400" placeholder="Preenchimento humano ou por agente solicitado no chat."/></label>;
const ContextCard = ({ contexts, partner }: { contexts: ProductContext[]; partner: string }) => { const items = contexts.filter((item) => !item.partner || item.partner === partner); return <section className="rounded-xl border border-slate-200 p-3"><h3 className="text-sm font-bold text-slate-900">Contextos de produto</h3>{items.length ? items.map((item) => <div key={item.id} className="mt-2 rounded-lg bg-slate-50 p-3"><div className="flex items-center justify-between"><b className="text-xs">{item.product}</b><span className="text-[10px] text-slate-500">v{item.version} · {item.provenance || 'sem fonte'}</span></div><p className="mt-1 text-xs text-slate-600">{item.valueProposition || 'Proposta de valor ainda não cadastrada.'}</p></div>) : <p className="mt-2 text-xs text-slate-500">Nenhum contexto governado para este parceiro.</p>}</section>; };
const GuardrailList = ({ contexts, guardrails, partner }: { contexts: ProductContext[]; guardrails: ProductGuardrail[]; partner: string }) => { const ids = new Set(contexts.filter((item) => !item.partner || item.partner === partner).map((item) => item.id)); const items = guardrails.filter((item) => ids.has(item.productContextId)); return <section className="rounded-xl border border-slate-200 p-3"><h3 className="text-sm font-bold text-slate-900">Guardrails vigentes</h3>{items.length ? items.map((item) => <div key={item.id} className={`mt-2 rounded-lg border p-3 ${item.severity === 'hard_block' ? 'border-red-200 bg-red-50' : item.severity === 'requires_review' ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}><div className="flex items-center justify-between gap-2"><b className="text-xs">{item.title}</b><span className="text-[9px] font-bold uppercase">{item.severity.replace('_', ' ')}</span></div><p className="mt-1 text-xs text-slate-700">{item.ruleText}</p>{item.evidence && <p className="mt-1 text-[10px] text-slate-500">Evidência: {item.evidence}</p>}</div>) : <p className="mt-2 text-xs text-slate-500">Nenhum guardrail cadastrado. Isso não significa liberação automática de claims.</p>}</section>; };
