...
export const DynamicEmailWorkspace: React.FC = () => {
  ...
  return <div className="min-h-full bg-slate-50 p-4 lg:p-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-8 gap-y-4">
        ...
        <div className="ml-auto flex flex-col items-end gap-2">
          ...
          {mode === 'briefings' && <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <HeaderAction onClick={() => setNewOpen(true)} icon={<Plus size={15}/>} label="Novo"/>
    ...
    {newOpen && <NewBriefingDialog groups={editorialGroups.filter((group) => group.visibleRows.length)} settings={signatureSettings} defaultSequence={`E-mail ${activeEditorialGroupCount + 1}`} onClose={() => setNewOpen(false)} onCreate={createBriefing}/>}
...
const BriefingTree = ({ groups, selectedId, showArchived, onSelect, onManage }: { groups: EditorialGroup[]; selectedId: string; showArchived: boolean; onSelect: (id: string) => void; onManage: (groupId: string) => void }) => {
...
const NewBriefingDialog = ({ groups, settings, defaultSequence, onClose, onCreate }: { groups: EditorialGroup[]; settings: SignatureSetting[]; defaultSequence: string; onClose: () => void; onCreate: (config: NewBriefingConfig) => void }) => {
  ...
  const [segment, setSegment] = useState('CRM'); const [weekKey, setWeekKey] = useState('Semana 1'); const [sequence, setSequence] = useState(defaultSequence); const [sourceGroupId, setSourceGroupId] = useState(''); const [signatureKeys, setSignatureKeys] = useState<string[]>(available.map((item) => item.key));
