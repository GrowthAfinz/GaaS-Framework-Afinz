  ...
  return <div className="min-h-full bg-slate-50 px-4 py-5 text-slate-800">
    <div className="mx-auto flex max-w-[1780px] flex-col gap-4">
      ...
      <section className="order-2 rounded-xl border border-slate-200 bg-white p-5">
        ...
        {metricMode === 'volume' && <div className="mt-2 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-2"><span className="mr-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Linhas de taxa</span>{rateConfig.map(rate => <button key={rate.key} title={rate.formula} onClick={() => toggleRate(rate.key)} className={`flex items-center gap-2 rounded-full border px-2.5 py-1 text-[10px] font-semibold transition ${selectedRates.includes(rate.key) ? 'border-slate-300 bg-white text-slate-800 shadow-sm' : 'border-slate-200 text-slate-400'}`}><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: selectedRates.includes(rate.key) ? rate.color : '#cbd5e1' }} />{rate.label}</button>)}<span className="text-[10px] text-slate-400">máx. 4</span></div>}
        <div className="mt-3 h-[380px]">
          ...
          <ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{ top: 18, right: metricMode === 'volume' ? 42 : 28, left: 8, bottom: granularity === 'daily' ? 20 : 4 }} barGap={2} barCategoryGap="22%"><CartesianGrid stroke="#dbe3ec" strokeDasharray="3 3" /><XAxis dataKey="period" height={granularity === 'daily' ? 42 : 30} axisLine={{ stroke: '#64748b' }} tickLine={{ stroke: '#64748b' }} tick={granularity === 'daily' ? ({ x, y, payload }: any) => { const point = chartData.find(item => item.period === payload.value); return <g transform={`translate(${x},${y})`}><text y={12} textAnchor="middle" fill="#334155" fontSize={10} fontFamily="ui-monospace, monospace" fontWeight={600}>{payload.value}</text><text y={26} textAnchor="middle" fill="#94a3b8" fontSize={9}>{point?.weekday}</text></g>; } : { fontSize: 10, fontFamily: 'ui-monospace, monospace', fill: '#475569' }} /><YAxis yAxisId="volume" axisLine={{ stroke: '#64748b' }} tickLine={{ stroke: '#64748b' }} tick={{ fontSize: 10, fontFamily: 'ui-monospace, monospace', fill: '#475569' }} tickFormatter={value => metricMode === 'rates' ? `${value}%` : value >= 1000 ? `${Math.round(value / 1000)} mil` : String(value)} /><YAxis yAxisId="rate" orientation="right" hide={metricMode === 'rates'} domain={[0, 'auto']} axisLine={{ stroke: '#64748b' }} tickLine={{ stroke: '#64748b' }} tick={{ fontSize: 10, fontFamily: 'ui-monospace, monospace', fill: '#475569' }} tickFormatter={value => `${value}%`} /><Tooltip cursor={{ fill: '#f1f5f9' }} contentStyle={{ border: '1px solid #cbd5e1', borderRadius: 2, fontSize: 11, fontFamily: 'ui-monospace, monospace' }} labelStyle={{ color: '#0f172a', fontWeight: 700, marginBottom: 6 }} formatter={(value: number, name: string) => { const rate = rateConfig.find(item => item.key === name); return [rate ? percentageLabel(value, 2) : formatNumber(value), rate ? `${rate.label} · ${rate.formula}` : stageConfig.find(stage => stage.key === name)?.label ?? name]; }} />{metricMode === 'volume' ? <>{stageConfig.filter(stage => selectedStages.includes(stage.key)).map(stage => <Bar key={stage.key} yAxisId="volume" dataKey={stage.key} name={stage.key} fill={stage.color} maxBarSize={granularity === 'daily' ? 18 : 38} />)}{rateConfig.filter(rate => selectedRates.includes(rate.key)).map(rate => <Line key={rate.key} yAxisId="rate" type="linear" dataKey={rate.key} name={rate.key} stroke={rate.color} strokeWidth={3} connectNulls={false} dot={{ r: 3.5, fill: '#fff', strokeWidth: 2.5 }} activeDot={{ r: 5.5 }} />)}</> : rateConfig.filter(rate => selectedRates.includes(rate.key)).map(rate => <Line key={rate.key} yAxisId="volume" type="linear" dataKey={rate.key} name={rate.key} stroke={rate.color} strokeWidth={3} connectNulls={false} dot={{ r: 3.5, fill: '#fff', strokeWidth: 2.5 }} activeDot={{ r: 5.5 }} />)}</ComposedChart></ResponsiveContainer>}
        </div>
        <p className="text-[10px] text-slate-500">{metricMode === 'volume' ? 'Barras/eixo esquerdo: volumes absolutos · linhas/eixo direito: taxas selecionadas.' : 'Conversões medem avanço do funil; coberturas usam Pedidos como base e não representam etapas obrigatoriamente sequenciais.'}</p>
      </section>
