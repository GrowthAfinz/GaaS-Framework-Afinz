  ...
  return (
    <div className="flex flex-wrap items-center gap-7 rounded-2xl bg-gradient-to-br from-[#063b3d] via-[#0a5f63] to-[#00838a] px-6 py-5 text-white">
      <div className="flex flex-shrink-0 items-center gap-5">
        ...
        <div>
          ...
          <div className="max-w-[380px] text-xs leading-snug text-white/80">
            {int(c.orphanExec)} execuções em {c.orfaos} disparos ainda sem template · {c.fortes} com match forte prontos para vincular
          </div>
        </div>
      </div>
      ...
      <div className="ml-auto flex items-stretch gap-3.5">
        ...
        <Stat v={c.semAsset} l="Aguardando asset" s="templates sem peça" tone="info" />
        ...
        <div className="flex flex-col justify-center gap-1.5 pl-1.5">
          {c.byChannel.map((x) => (
            <div key={x.ch} className="flex items-center gap-2 text-[11.5px]" title={`${x.label}: ${x.orf} órfãos · ${x.ass}/${x.tpl} com asset`}>
              <span className="h-2 w-2 rounded-full" style={{ background: x.color }} />
              <span className="min-w-[64px] text-white/85">{x.label}</span>
              <span className="font-semibold text-white/60">{x.orf > 0 ? `${x.orf} órfãos` : 'ok'}</span>
            </div>
          ))}
        </div>
      </div>
