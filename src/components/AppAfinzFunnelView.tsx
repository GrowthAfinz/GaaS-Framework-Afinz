...
export const AppAfinzFunnelView: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([key, rows]) => {
      ...
      return { period: label, weekday: weekday(first), ...values, completionRate: pct(values.completed, scope === 'total' ? values.onboarding : values.geo) };
    });
