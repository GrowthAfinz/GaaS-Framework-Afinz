...
function buildDaily(raw: RawRow[], scope: Scope): DailyRow[] {
  ...
  return dates.map(key => {
    ...
    source.forEach(row => {
      ...
      else if (scope === 'total' && splitMeasure && stage !== 'onboarding') result[stage] = (result[stage] ?? 0) + row.value;
      else if (scope !== 'total' && splitMeasure && row.plurix === (scope === 'plurix' ? 'SIM' : 'NAO') && stage !== 'onboarding') result[stage] = row.value;
    });
    return result;
  });
}
...
export const AppAfinzFunnelView: React.FC = () => {
  ...
  const availableRates = rates.filter(rate => {
    const crossesUnsegmentedEntry = scope !== 'total' && (rate.key === 'onboardingRate' || rate.key === 'geoRate' || rate.key === 'overallRate');
    return !crossesUnsegmentedEntry && compatibleDailyRate(totals[rate.numerator], totals[rate.denominator]) != null;
  });
  ...
  return <div className="min-h-full bg-slate-50 px-4 pb-6 text-slate-800">
    <div className="mx-auto flex max-w-[1780px] flex-col gap-4">
      ...
      <section className="border border-slate-200 bg-white p-4">
        <div className="flex items-end justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-950">Funil completo · {scopeLabel}</h2><p className="text-xs text-slate-500">Todas as etapas disponíveis são preservadas; taxas só conectam populações compatíveis.</p></div><p className="text-right"><span className="block text-[9px] uppercase tracking-wide text-slate-400">{scope === 'total' ? 'Conversão geral' : 'Conclusão após geolocalização'}</span><strong className="font-mono text-xl text-emerald-700">{pctLabel(completedRate)}</strong></p></div>
        <div className={`mt-3 grid gap-1.5 ${cardStages.length > 7 ? 'lg:grid-cols-9' : 'lg:grid-cols-7'} sm:grid-cols-3`}>{cardStages.map((stage, index) => { const referenceOnly = scope !== 'total' && (stage.key === 'cpf' || stage.key === 'onboarding'); const incompatibleTransition = scope !== 'total' && stage.key === 'geo'; const prior = index > 0 && !incompatibleTransition ? totals[cardStages[index - 1].key] : null; const rate = index > 0 && !referenceOnly && !incompatibleTransition ? pct(totals[stage.key], prior) : null; return <div key={stage.key} className={`border px-3 py-2.5 ${referenceOnly ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}><div className="flex items-center justify-between"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-200 text-[9px] font-bold">{index + 1}</span><span className="h-2 w-2" style={{ backgroundColor: stage.color }} /></div><p className="mt-2 min-h-[28px] text-[10px] font-semibold text-slate-700">{stage.label}</p><p className="font-mono text-lg font-bold text-slate-950">{fmt(totals[stage.key])}</p><p className={`text-[9px] font-semibold ${referenceOnly ? 'text-amber-800' : 'text-cyan-700'}`}>{referenceOnly ? 'referência do App total' : incompatibleTransition ? 'entrada segmentada disponível daqui' : rate == null ? 'volume do período' : `${pctLabel(rate)} da etapa anterior`}</p></div>; })}</div>
