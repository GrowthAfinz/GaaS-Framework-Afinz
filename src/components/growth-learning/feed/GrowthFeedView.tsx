...
export const GrowthFeedView: React.FC<GrowthFeedViewProps> = ({ periodStart, periodEnd, onCountChange }) => {
  ...
  const activeFilterLabels = [
    filters.front !== 'all' ? ({ crm_acquisition: 'CRM Aquisição', paid_media: 'Mídia Paga', b2c_origin: 'Originação B2C', report_live: 'Report Live' } as Record<string, string>)[filters.front] : null,
  ...
  const emptyMessage = filters.front === 'b2c_origin'
    ? 'Originação B2C ainda não possui um produtor governado de eventos. A frente permanece visível, sem conteúdo simulado.'
    : events.length === 0
      ? 'Nenhum produtor sistêmico registrou evento neste período.'
      : 'Há eventos no período, mas nenhum corresponde aos filtros ativos.';
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-cyan-700"><Inbox size={15} /> Fila sistêmica</div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400"><Filter size={13} /> Filtros</span>
          <select aria-label="Filtrar por frente" value={filters.front} onChange={(e) => changeFilters({ front: e.target.value as GrowthFeedFilters['front'] })} className={selectClass}>
            <option value="all">Todas as frentes</option><option value="crm_acquisition">CRM Aquisição</option><option value="paid_media">Mídia Paga</option><option value="b2c_origin">Originação B2C</option><option value="report_live">Report Live</option>
          </select>
          <select aria-label="Filtrar por confiança" value={filters.confidence} onChange={(e) => changeFilters({ confidence: e.target.value as GrowthFeedFilters['confidence'] })} className={selectClass}>
            <option value="all">Toda confiança</option><option value="confirmed">Confirmada</option><option value="directional">Direcional</option><option value="suspect">Sob suspeita</option><option value="blocked">Bloqueada</option>
          </select>
          <select aria-label="Filtrar por estado" value={filters.state} onChange={(e) => changeFilters({ state: e.target.value as GrowthFeedFilters['state'] })} className={selectClass}>
            <option value="all">Todos os estados</option><option value="open">Abertos</option><option value="blocked">Bloqueados</option><option value="certified">Certificados</option><option value="published">Publicados</option>
          </select>
          <span className="mx-1 hidden h-6 w-px bg-slate-200 lg:block" />
          <label className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400"><SlidersHorizontal size={13} /> Ordenar</label>
          <select aria-label="Ordenar fila" value={filters.sort} onChange={(e) => changeFilters({ sort: e.target.value as GrowthFeedFilters['sort'] })} className={selectClass}>
            <option value="priority">Prioridade</option><option value="recent">Recentes</option><option value="relevance">Relevância</option>
          </select>
        </div>
      </div>
      ...
      {activeFilterLabels.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 px-1">
          ...
          {activeFilterLabels.map((label) => <span key={label} className="rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-1 text-[11px] font-bold text-cyan-800">{label}</span>)}
          <button type="button" onClick={() => changeFilters({ front: 'all', confidence: 'all', state: 'all' })} className="text-xs font-bold text-slate-500 underline underline-offset-2">Limpar</button>
        </div>
      )}

      {loading ? (
        <div className="grid min-h-[340px] place-items-center rounded-2xl border border-slate-200 bg-white"><span className="flex items-center gap-2 text-sm font-semibold text-slate-500"><Loader2 size={18} className="animate-spin" /> Lendo eventos governados…</span></div>
      ) : error ? (
        <div className="grid min-h-[340px] place-items-center rounded-2xl border border-rose-200 bg-rose-50 px-6 text-center">
          <div><AlertCircle size={30} className="mx-auto text-rose-600" /><h3 className="mt-3 font-black text-rose-900">A fila não pôde ser carregada</h3><p className="mt-2 max-w-xl text-sm text-rose-800">{error}</p><button type="button" onClick={() => void load()} className="mx-auto mt-4 inline-flex items-center gap-2 rounded-xl bg-rose-900 px-4 py-2 text-xs font-bold text-white"><RefreshCw size={14} /> Tentar novamente</button></div>
        </div>
      ) : groups.length === 0 ? (
        <div className="grid min-h-[340px] place-items-center rounded-2xl border border-dashed border-slate-300 bg-white px-6 text-center">
          <div><Inbox size={32} className="mx-auto text-slate-300" /><h3 className="mt-3 font-black text-slate-800">Nada para mostrar nesta leitura</h3><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{emptyMessage}</p></div>
        </div>
      ) : (
        <div className="space-y-4">
          {groups.map((group) => (
            <GrowthFeedCard
              ...
              onToggleGroup={() => setExpandedGroups((current) => {
